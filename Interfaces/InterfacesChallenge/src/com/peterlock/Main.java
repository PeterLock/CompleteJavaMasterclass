...
public class Main {
    ...
    public static void main(String[] args) {
        ISaveable monster;
        ISaveable playerAlex;


        playerAlex = new Player("Alex Rig");
        System.out.println(playerAlex);

        save(playerAlex);

    }

    private static void save(ISaveable objectToWrite) {
        System.out.println("\nSaving objectToWrite . . .");
        for(int i = 0; i < objectToWrite.write().size(); i++){
            System.out.println("Saving " + "[" + objectToWrite.write().get(i) + "]" + " to storage device");
        }
    }
